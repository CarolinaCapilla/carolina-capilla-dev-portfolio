<template>
    <div class="group relative overflow-hidden rounded-lg" :class="cardClass">
        ...
        <div
            ...
            <template v-if="project.isPortfolio">
                <div class="flex flex-col items-center gap-2 text-white">
                    ...
                    <template v-if="project.source">
                        <a :href="project.source" target="_blank" rel="noopener" class="mt-2 flex items-center gap-2 text-sm font-semibold">
                            <Icon name="simple-icons:github" size="18" />
                            <span>View Source Code</span>
                        </a>
                    </template>
                    <template v-else>
                        <span class="mt-2 flex items-center gap-2 text-sm font-semibold opacity-90">
                            <Icon name="simple-icons:github" size="18" />
                            <span>View Source Code</span>
                        </span>
                    </template>
            ...
            <template v-else>
                <div class="flex flex-col items-center gap-2 text-white">
                    <a :href="project.link" target="_blank" rel="noopener" class="flex items-center gap-2 text-sm font-semibold">
        ...
        <div class="p-4">
            <h3 class="text-xl font-semibold" :class="textClass">{{ project.title }}</h3>
            <p class="text-neutral-700 dark:text-neutral-300 text-sm">{{ project.description }}</p>
            <div v-if="project.tags?.length" class="mt-3 flex flex-wrap gap-2">
                <span v-for="(tag, i) in project.tags" :key="i" :class="[chipClass, 'px-3 text-sm']">
                    <Icon v-if="tag.icon" :name="tag.icon" class="mr-1 inline-block align-[-0.125em] transition-opacity duration-200 group-hover:opacity-0" size="16" />
                    {{ tag.name }}
                </span>
            </div>
        </div>
